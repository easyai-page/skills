...
3. GitHub Actions 的 `release` workflow 自动执行：
   verify（版本一致性）→ build（7 平台并行：Linux/Windows/macOS 各含 x86_64 与 arm64，
   另加 linux-musl 静态版）→ release（校验和 + 中文安装说明 + 发布）。
4. 在 Releases 页确认 7 个归档 + `SHA256SUMS` + 中文 notes。
   每个归档内含中文安装脚本：Linux/macOS 用 `bash install.sh`，
   Windows 用 `powershell -ExecutionPolicy Bypass -File install.ps1`。
...
| 某个平台 build 失败 | 看该 job 日志修复代码；修复提交后删 tag 重打（同上）。其他平台的成功产物不保留——release 只在 7 个全绿时产出 |
