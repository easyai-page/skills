...
3. GitHub Actions 的 `ci` workflow 自动执行：
   test（三平台测试）→ verify（版本一致性）→ build（6 平台并行：Linux/Windows/macOS
   各含 x86_64 与 arm64，全部原生 runner）→ release（校验和 + 中文安装说明 + 发布）。
4. 在 Releases 页确认 6 个归档 + `SHA256SUMS` + 中文 notes。
